...
async function resolveTorboxLinkWithWait({
  ...
  try {
    ...
    const alreadyExists = isCreateAlreadyExistsError(error);
    if (alreadyExists) {
      // Already exists is OK; we'll locate it via mylist.
      createFailed = false;
    } else {
      // TorBox can reject problematic display names; retry once without the optional name.
      try {
        const createdWithoutName = await createTorrentFromMagnet({ apiKey, magnet, name: undefined });
        torrentId = pickTorrentId(createdWithoutName)
          || String(createdWithoutName?.data?.torrentId || createdWithoutName?.data?.torrent_id || '');
      } catch (retryError) {
        createFailed = true;
        console.error('TorBox createTorrent failed:', retryError?.message || retryError);
      }
    }
  }
