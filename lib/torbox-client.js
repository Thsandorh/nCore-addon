...
async function createTorrentFromMagnet({ apiKey, magnet, name, attempts = 1 }) {
  ...
  try {
    ...
    // Handle successful creation - return the data
    if (response?.success && response?.data) {
      return response.data;
    }

    // Check for ACTIVE_LIMIT error and retry with space freeing
    if (response?.error === 'ACTIVE_LIMIT' && attempts > 0) {
      const freed = await freeLastActiveTorrent({ apiKey });
      if (freed) {
        return createTorrentFromMagnet({ apiKey, magnet, name, attempts: attempts - 1 });
      }
    }

    // Return response data even if success flag is missing (some API responses vary)
    return response?.data || response;
  } catch (error) {
    // Handle ACTIVE_LIMIT errors that come as exceptions
    if (isTorboxQueueLimitError(error) && attempts > 0) {
      const freed = await freeLastActiveTorrent({ apiKey });
      if (freed) {
        return createTorrentFromMagnet({ apiKey, magnet, name, attempts: attempts - 1 });
      }
...
async function resolveTorboxLinkWithWait({
  ...
  // Step 1: Try to create or find the torrent
  let torrentId = '';
  let queuedId = '';

  try {
    const created = await createTorrentFromMagnet({ apiKey, magnet, name: fileName, attempts: 1 });
    torrentId = pickTorrentId(created) || '';
    queuedId = pickQueuedId(created) || '';
  } catch (error) {
    // If torrent already exists, we'll find it in the list below
    if (!isCreateAlreadyExistsError(error)) {
      // For queue limit errors, the error is already handled in createTorrentFromMagnet
  ...
  // Step 2: Poll TorBox mylist until torrent is ready
