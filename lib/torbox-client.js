...
function findTorrentById(torrents, torrentId) {
  const wanted = String(torrentId || '');
  if (!wanted) return null;
  return torrents.find((torrent) => getTorrentId(torrent) === wanted) || null;
}
...
async function resolveLink({ apiKey, magnet, infoHash, preferredFile, season, episode, maxWaitMs = 15000 }) {
  ...
  const resolveFromList = async () => {
    ...
    const torrent = findTorrentByHash(torrents, hash) || findTorrentById(torrents, torrentId);
    if (!torrent) return '';
    ...
    try {
      return await getDownloadLink({ apiKey, torrentId, fileId });
    } catch {
      if (!isTorrentReady(torrent)) return '';
      return '';
    }
  };
