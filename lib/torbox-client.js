...
function chooseFileIdFromTorrent(item, preferredName) {
...
async function createTorrentFromMagnet({ apiKey, magnet }) {
  const attempts = [
    ...
    { body: JSON.stringify({ link: magnet }), headers: { 'content-type': 'application/json' } },
    ...
    { body: new URLSearchParams({ link: magnet }).toString(), headers: { 'content-type': 'application/x-www-form-urlencoded' } },
  ];
...
async function resolveTorboxLink({ apiKey, magnet, infoHash, fileName }) {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    const listPayload = await fetchJsonWithAuth({
    ...
    if (chosen) {
      ...
      const fileId = chooseFileIdFromTorrent(chosen, fileName);
      if (torrentId && fileId) {
        const url = await requestDownloadLink({ apiKey, torrentId, fileId });
        const subtitleCandidates = pickSubtitleFiles(chosen, fileName, 4);
        const subtitles = [];

        for (let idx = 0; idx < subtitleCandidates.length; idx += 1) {
          const subtitle = subtitleCandidates[idx];
          try {
            const subtitleUrl = await requestDownloadLink({ apiKey, torrentId, fileId: subtitle.id });
            subtitles.push({
