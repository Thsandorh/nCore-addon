...
async function ensureTorrentQueued({ apiKey, magnet, name, waitMs = 20_000, pollIntervalMs = 1500 }) {
  ...
  const normalizedName = String(name || readNameFromMagnet(magnet) || '').replace(/\s+/g, ' ').trim().slice(0, 180);
  ...
  for (let index = 0; index < createAttempts.length; index += 1) {
    const attemptName = createAttempts[index];
    try {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      const found = findTorrentInList(list, infoHash, torrentId);
      if (found) {
        return {
          queued: true,
          torrentId: pickTorrentId(found) || torrentId,
          queuedId: String((found?.queued_id ?? found?.queuedId ?? queuedId) || ''),
          infoHash: readTorboxInfoHash(found) || infoHash,
          state: readTorboxState(found),
        };
      }
    } catch {
      // ignore transient list errors during queue verification
    }
