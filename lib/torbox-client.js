...
async function createTorrentFromMagnet({ apiKey, magnet, name }) {
  ...
  // Removed as_queued and seed parameters to match Torrentio behavior
  // This allows uncached torrents to be submitted properly
...
function statusSeeding(item) {
  if (!item || typeof item !== 'object') return false;
  const state = readTorboxState(item);
  return ['seeding', 'uploading', 'uploading (no peers)'].includes(state);
}
...
async function controlTorrent({ apiKey, torrentId, operation }) {
  ...
  const form = createFormData();
  form.append('torrent_id', String(torrentId));
  form.append('operation', String(operation));

  return fetchJsonWithAuth({
    path: '/api/torrents/controltorrent',
    method: 'POST',
    apiKey,
    body: form,
  });
...
async function freeLastActiveTorrent({ apiKey }) {
  ...
  const list = await listMyTorrents({ apiKey, bypassCache: true });

  // Try to stop seeding torrents first (less destructive)
  const seedingTorrents = list.filter(statusSeeding);
  if (seedingTorrents.length > 0) {
    const torrent = seedingTorrents[seedingTorrents.length - 1];
    const torrentId = pickTorrentId(torrent);
    if (torrentId) {
      console.log(`TorBox: Stopping seeded torrent ${torrentId} to make space...`);
      await controlTorrent({ apiKey, torrentId, operation: 'stop_seeding' });
      return true;
    }
  }

  // If no seeding torrents, try to delete downloading torrents
  const downloadingTorrents = list.filter(statusDownloading);
  if (downloadingTorrents.length > 0) {
    const torrent = downloadingTorrents[downloadingTorrents.length - 1];
    const torrentId = pickTorrentId(torrent);
    if (torrentId) {
      console.log(`TorBox: Deleting downloading torrent ${torrentId} to make space...`);
    ...
    if (!createdAttempted) {
      ...
      try {
        ...
        if (isCreateAlreadyExistsError(error)) {
          ...
          console.log('TorBox: Queue limit reached, attempting to free space...');
          try {
            const freed = await freeLastActiveTorrent({ apiKey });
            if (freed) {
              console.log('TorBox: Space freed, retrying torrent submission...');
              const retryCreated = await createTorrentFromMagnet({ apiKey, magnet, name: fileName });
              ...
              console.log('TorBox: Could not free space, queue limit error');
              error.code = readTorboxErrorCode(error) || 'ACTIVE_LIMIT';
              throw error;
            }
          } catch (freeError) {
            if (isTorboxQueueLimitError(freeError)) {
              freeError.code = readTorboxErrorCode(freeError) || 'ACTIVE_LIMIT';
              throw freeError;
            }
            // If freeing failed for other reasons, throw original error
            error.code = readTorboxErrorCode(error) || 'ACTIVE_LIMIT';
            throw error;
          }
        } else {
          ...
          try {
            ...
            if (isTorboxQueueLimitError(retryError)) {
              // Try to free space and retry one more time
              console.log('TorBox: Queue limit reached on retry, attempting to free space...');
              try {
                const freed = await freeLastActiveTorrent({ apiKey });
                if (freed) {
                  console.log('TorBox: Space freed, retrying torrent submission...');
                  const finalRetry = await createTorrentFromMagnet({ apiKey, magnet, name: undefined });
                  torrentId = pickTorrentId(finalRetry)
                    || String(finalRetry?.data?.torrentId || finalRetry?.data?.torrent_id || '');
                  queuedId = pickQueuedId(finalRetry);
                } else {
                  retryError.code = readTorboxErrorCode(retryError) || 'ACTIVE_LIMIT';
                  throw retryError;
                }
              } catch (finalError) {
                if (isTorboxQueueLimitError(finalError)) {
                  finalError.code = readTorboxErrorCode(finalError) || 'ACTIVE_LIMIT';
                  throw finalError;
                }
                retryError.code = readTorboxErrorCode(retryError) || 'ACTIVE_LIMIT';
                throw retryError;
