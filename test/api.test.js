...
test('resolve refreshes nCore data and adds the refreshed selected magnet', async () => {
  const firstHash = '2222222222222222222222222222222222222222';
  const refreshedHash = '3333333333333333333333333333333333333333';
  const firstMagnet = `magnet:?xt=urn:btih:${firstHash}&dn=old`;
  const refreshedMagnet = `magnet:?xt=urn:btih:${refreshedHash}&dn=new`;
  ...
  const app = createApp({
    ...
    searchClient: async () => {
      searchCalls += 1;
      if (searchCalls === 1) {
        return [{ id: '77', title: 'Test Movie', magnet: firstMagnet, infoHash: firstHash }];
      }
      return [{ id: '77', title: 'Test Movie', magnet: refreshedMagnet, infoHash: refreshedHash }];
    },
    torboxCachedChecker: async () => new Map(),
    torboxAdder: async ({ magnet }) => { addedMagnet = magnet; },
    torboxResolver: async () => 'https://cdn.example.test/new.mp4',
  });
  ...
  assert.equal(resolveResponse.status, 302);
  assert.equal(addedMagnet, refreshedMagnet);
