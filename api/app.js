...
function findRefreshedSelectionResult(results, selection) {
  const list = Array.isArray(results) ? results : [];
  if (!list.length) return null;

  if (selection.ncoreId) {
    const byId = list.find((item) => String(item?.id || '') === String(selection.ncoreId));
    if (byId) return byId;
  }

  if (selection.infoHash) {
    const byHash = list.find((item) => deriveInfoHash(normalizeMagnet(item?.magnet), item?.infoHash) === selection.infoHash);
    if (byHash) return byHash;
  }

  if (selection.title) {
    const wantedTitle = String(selection.title).toLowerCase();
    const byTitle = list.find((item) => String(item?.title || '').toLowerCase() === wantedTitle);
...
async function refreshSelectionFromNcore({ selection, credentials, searchClient }) {
  if (!selection?.queryId) return selection;

  const results = await searchClient({
    username: credentials.username,
    password: credentials.password,
    query: selection.queryId,
  });

  const picked = findRefreshedSelectionResult(results, selection);
  if (!picked) return selection;

  const magnet = normalizeMagnet(picked.magnet);
  const infoHash = deriveInfoHash(magnet, picked.infoHash);
  if (!magnet || !infoHash) return selection;

  selection.magnet = magnet;
  selection.infoHash = infoHash;
  selection.title = picked.title || selection.title;
  selection.ncoreId = String(picked.id || selection.ncoreId || '');
  if (!(selection.season && selection.episode)) {
    selection.preferredFile = picked.fileName || selection.preferredFile;
  }
  return selection;
}
  ...
  return async function app(req, res) {
    ...
    if ((req.method === 'GET' || req.method === 'HEAD') && resolveMatch) {
      ...
      try {
        ...
        await refreshSelectionFromNcore({ selection, credentials, searchClient }).catch(() => selection);
      ...
      try {
        ...
        for (const item of results.slice(0, 30)) {
          ...
          selectionStore.set(selectionId, {
            ...
            magnet,
            infoHash,
            ncoreId: String(item.id || ''),
            title: item.title || '',
            queryId: parsedId.raw,
